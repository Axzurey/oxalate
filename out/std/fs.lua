...
local function resolve_to_string(path)
	local path_buf = {}
	local first = true
	...
	return table.concat(path_buf, "/")
end
local function get_from_resolved_path(resolvedstr)
	local resolved = string.split(resolvedstr, "/")
	local current = game
	do
		local i = 1
